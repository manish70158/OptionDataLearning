...
def reclassify_views(df):
    df = df.copy()
    df["fii_view_new"] = df["fii_composite"].apply(classify_composite)
    df["pro_view_new"] = df["pro_composite"].apply(classify_composite)
    return df
...
def generate_header(df):
    ...
    lines = [
        "# FII VIEW x PRO VIEW — All Combinations Analysis (No Neutral Zone)",
        "",
        ...
        "- **Bullish / Bearish** = composite ±50K to ±100K (fixed across all thresholds)",
        "- **Strong Bullish / Strong Bearish** = composite beyond ±100K (fixed across all thresholds)",
        "- **No Neutral zone** — every day is classified as directional (bullish or bearish)",
    ]
    return "\n".join(lines)
...
def generate_fii_sections(stats, df):
    lines = []
    section_num = 1
...
def generate_conclusion(stats, df):
    filtered = stats[stats["days"] >= 5].copy()
    total_days = len(df)
    ...
    lines = ["## Conclusion", ""]
    lines.append("### Analysis with No Neutral Zone")
    lines.append("")
    lines.append(f"**Distribution impact**: With no neutral zone, {mild_bull} days are classified as FII Mildly Bullish "
                 f"(composite 0 to +50K) and {mild_bear} as FII Mildly Bearish (composite -50K to 0). "
                 f"Every day carries a directional signal.")
    lines.append("")
    ...
    # Note about no-neutral impact
    lines.append(f"{finding_num}. **No-neutral impact**: By eliminating the neutral zone, the Mildly Bullish and Mildly Bearish "
                 f"categories absorb all previously neutral days. This reveals whether low-conviction directional signals "
                 f"still carry predictive value.")
    finding_num += 1
...
def generate_report(output_path):
    ...
    print(f"Generated: {output_path} (no neutral zone, {len(df)} days, {len(stats)} combinations)")


def main():
    generate_report(str(REPORTS_DIR / "FII_PRO_NO_NEUTRAL_DIRECTIONAL_ANALYSIS.md"))
    print("Done! Directional report generated in reports_no_neutral/ directory.")
