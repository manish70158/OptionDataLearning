...
OUTPUT_PATH = SCRIPT_DIR / "FII_PRO_VOLATILITY_WHIPSAW_ANALYSIS.md"

NEUTRAL_THRESHOLD = 20000  # Use ±20K as the default classification
...
def load_and_prepare(vix_fraction=0.5):
    ...
    df["fii_view"] = df["fii_composite"].apply(lambda v: classify_composite(v, NEUTRAL_THRESHOLD))
    df["pro_view"] = df["pro_composite"].apply(lambda v: classify_composite(v, NEUTRAL_THRESHOLD))
...
def generate_report(df, intra_df, vix_fraction=0.5, output_path=None):
    ...
    # === HEADER ===
    sections.append(f"""# FII VIEW x PRO VIEW — Volatility & Whipsaw Analysis ({frac_label} Threshold)

**Dataset**: {total_days:,} days | {date_min} to {date_max} | ±{TK}K Neutral Classification
...
### Label Guide
- **Neutral{TK}** = composite between ±{TK}K (i.e. -{TK}K to +{TK}K)
- **MildBull{TK}** = Mildly Bullish = composite +{TK}K to +50K
- **MildBear{TK}** = Mildly Bearish = composite -{TK}K to -50K
...
- Average Up Ratio: {df['up_ratio'].mean():.2f} | Average Down Ratio: {df['down_ratio'].mean():.2f}""")
    ...
    # Only for whipsaw extreme days that have intraday data
    ...
    if not iron_condor.empty:
        ...
        lines.append("- **Iron condor / range-bound candidates**: Most combos exceed VIX prediction; use MildlyBullish+MildlyBullish (lowest whipsaw+range/VIX ratio) or Bullish+StrongBearish")
    ...
    with open(out, "w") as f:
        f.write(report)
    print(f"Generated: {out} ({frac_label} threshold)")
...
def main():
    ...
    generate_report(df_half, intra_df, vix_fraction=0.5,
                    output_path=str(SCRIPT_DIR / "FII_PRO_VOLATILITY_WHIPSAW_ANALYSIS.md"))

    # 0.3×VIX report
    df_03 = load_and_prepare(vix_fraction=0.3)
    generate_report(df_03, intra_df, vix_fraction=0.3,
                    output_path=str(SCRIPT_DIR / "FII_PRO_VOLATILITY_WHIPSAW_0.3VIX_ANALYSIS.md"))

    print("Done! Both volatility reports generated.")
