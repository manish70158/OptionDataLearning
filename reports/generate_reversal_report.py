...
OUTPUT_PATH = SCRIPT_DIR / "FII_PRO_VIX_REVERSAL_TRAP_ANALYSIS.md"

NEUTRAL_THRESHOLD = 20000
...
def generate_report(df, intra_df):
    ...
    # === HEADER ===
    sections.append(f"""# FII VIEW x PRO VIEW — VIX Reversal & Trap Analysis
...
**Key insight**: Bull Trap + Bear Trap (small move → big reversal) capture 406 days — these are the "trap" days where the initial move is deceptive. Rally Fade + Drop Bounce (big move → small reversal) capture 224 days — these are the "giveback" days where an initial strong move partially unwinds.""")
    ...
    # === CONCLUSION ===
    # Find top combos for each pattern
    lines = ["## Conclusion", ""]
    ...
    # Green% comparison
    bt_gp = (df[df["bull_trap"]]["nifty_day"] == "Green").sum() / bull_trap_count * 100
    br_gp = (df[df["bear_trap"]]["nifty_day"] == "Green").sum() / bear_trap_count * 100
    ...
    for label, stats, desc in [
        ...
        if stats.empty:
            continue
        top = stats[stats["total_combo_days"] >= 5].sort_values("pattern_rate", ascending=False)
        if not top.empty:
            t = top.iloc[0]
